...
- ✏️ **动态编辑**：修改已发布内容
- 🏷️ **动态标签**：每位用户维护自己的标签候选项，发布/编辑时可选择或新建标签；书友圈支持按标签和发表用户搜索
- 🔒 **可见性控制**：公开/仅自己可见/好友可见
...
### 安全与权限
- 🔐 **Supabase Auth**：注册/登录；登录拼图支持鼠标、触摸和键盘，失败可立即重试，登录重试保留本页已通过的验证。拼图仅为前端交互校验，不替代服务端防机器人保护。
...
登录页的拼图验证支持鼠标、触摸和键盘操作。换图或重新进入登录页时会清理旧事件；拖动失败可以立即重试，不再锁定五分钟；邮箱或密码错误会保留本页已通过的验证，网络或服务异常不会计入密码错误。登录按钮在请求完成前会禁用，避免重复提交。浏览器本地的拼图只负责交互校验，不替代服务端 CAPTCHA 或机器人防护。
...
`npm test` 覆盖换图、页面重入、拖动取消、键盘操作、窄容器重算、重复提交、登录错误分类和本地冷却逻辑。正式环境还应使用测试账号手动检查正常拖动、拖错后立即重试、换图后拖动、错误密码重试、退出后重新进入，以及手机触摸和横竖屏变化。
...
本次登录验证修复只涉及前端代码和测试，不需要新增数据库迁移或 Edge Function。发布到 Cloudflare Pages 后，请用无痕窗口或强制刷新打开 `#/login`，确认页面加载了最新的 `src/captcha.js`、`src/authPages.js` 和 `src/styles.css`，再完成上一节的正式环境手动验收。手机 PWA 需要关闭旧窗口后重新打开，以便获取最新前端资源。
...
2026-08-18 视觉复核后修正了 standalone 手机模式的横向内边距：PWA 专属 `.section` 不再用 shorthand `padding` 覆盖全局 `.container`，并由 `.container.section` 显式恢复左右留白，书库、书友圈榜单、标题和卡片在窄屏下保持可读间距；同时隐藏与底部“我的”重复的顶部“个人中心”入口。普通浏览器继续使用原有样式。
...
- **2026-09-05**：修复登录拼图验证在换图、页面重入和触摸中断时的状态失效；增加鼠标/触摸/键盘交互、失败即时重试、重复提交保护和登录错误分类；补充 14 项自动化回归测试及正式环境验收说明。
...
- **2026-08-18**：PWA 移动端视觉复核：补齐 standalone 模式 `.container.section` 的显式左右留白，避免书库、书友圈榜单等页面贴边；隐藏顶部重复的“个人中心”入口，保留底部“我的”导航，并更新 PWA 样式版本号以触发已安装窗口重新获取 CSS。
- **2026-08-21**：书友圈新增按用户隔离的动态标签；发布和编辑时可以选择已用标签或新建标签，搜索支持全部内容、标签、发表用户和书名/作者范围；当前 Supabase 项目已执行 v52 迁移。
- **2026-08-01**：浏览券改为 Supabase Cron 定时发放：每周日北京时间 20:00 核算当周阅读贡献并自动发券，后台不再需要手动操作。
- **2026-08-01**：部署现状更新为 Cloudflare Pages + 已购 `.com` 自定义域名；数据库和后端服务仍在 Supabase。当前未使用 Cloudflare 或 Supabase 的付费服务；当时手机端尚未进行专项优化，后续已在 2026-08-18 完成 standalone PWA 本地实现。
