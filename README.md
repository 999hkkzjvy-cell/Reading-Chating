...
2026-08-18 视觉复核后修正了 standalone 手机模式的横向内边距：PWA 专属 `.section` 不再用 shorthand `padding` 覆盖全局 `.container` 的左右留白，书库、书友圈榜单、标题和卡片在窄屏下保持可读间距；同时隐藏与底部“我的”重复的顶部“个人中心”入口。普通浏览器继续使用原有样式。
...
- **2026-08-18**：PWA 移动端视觉复核：修正 standalone 模式 `.container` 左右留白被覆盖的问题，避免书库、书友圈榜单等页面贴边；隐藏顶部重复的“个人中心”入口，保留底部“我的”导航。
